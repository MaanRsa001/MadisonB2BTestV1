...
public class ReportBean extends MyJdbcTemplate 
{
	...
	public String[][] getPortfolioByDate(String eDate, String type,String status, String pid) 
	{
		...
		try {
			...
			ss = runner.multipleSelection(sql);
			runner.multipleSelection(sqlMultiple);
		} 
		catch (Exception e) 
		{
			System.out.println("Exception in gettin refferal data"	+ e.toString());
...
//not used
	...
	public String[][] getOpencoverList()	
	{
			String depositInfo[][] = new String[0][0];

			String sql ="SELECT OCM.PROPOSAL_NO ,OCM.MISSIPPI_OPENCOVER_NO ,to_char(OCM.POLICY_START_DATE,'dd/mm/yyyy') ,PI.FIRST_NAME FROM OPEN_COVER_MASTER OCM,OPEN_COVER_POSITION_MASTER OCPM , ( SELECT DISTINCT PROPOSAL_NO FROM OPEN_COVER_MASTER WHERE DEPOSIT_PREMIUM_YN = 'Y' ) OCM1 ,PERSONAL_INFO PI WHERE OCPM.STATUS='P' AND OCPM.PROPOSAL_NO = OCM.PROPOSAL_NO AND OCM.PROPOSAL_NO = OCM1.PROPOSAL_NO AND OCM.AMEND_ID = ( SELECT MAX(AMEND_ID) FROM OPEN_COVER_MASTER WHERE PROPOSAL_NO=OCM.PROPOSAL_NO ) AND PI.CUSTOMER_ID = OCM.CUSTOMER_ID  AND PI.AMEND_ID = ( select max(amend_id) from personal_info where customer_id = pi.customer_id ) AND PI.status='Y'";
			try{
				depositInfo = runner.multipleSelection(sql);
			}
			catch(Exception e){e.printStackTrace();}
			return depositInfo;
	}
	public String[][] getOpencover(String opencoverNo)	
	{
			String depositInfo[][] = new String[0][0];

			String sql ="SELECT SNO,to_char(STARTDATE,'dd/mm/yyyy'),to_char(ENDDATE,'dd/mm/yyyy') FROM TABLE(GET_OPENCOVER_TERM(?))";
			try{
				depositInfo = runner.multipleSelection(sql,new String[]{opencoverNo});
			}
			catch(Exception e){e.printStackTrace();}
			return depositInfo;
	}
	public String[][] getDepositOpencover(String opencoverNo,String startDate,String endDate)	
	{
			String depositInfo[][] = new String[0][0];

			String sql ="SELECT POLICY_NO ,to_char(PM.INCEPTION_DATE,'dd/mm/yyyy') ,MD.PREMIUM-NVL(POLICY_FEE,0)+NVL(PM.EXCESS_PREMIUM,0) PREMIUM,NVL(POLICY_FEE,0) ISSUANCE_FEE, MD.PREMIUM+NVL(PM.EXCESS_PREMIUM,0) TOTAL_PREMIUM FROM POSITION_MASTER PM, MARINE_DATA MD WHERE  PM.OPEN_COVER_NO = ? AND PM.APPLICATION_NO=MD.APPLICATION_NO AND TRUNC(PM.INCEPTION_DATE) BETWEEN TO_DATE(?,'dd/mm/yyyy')  AND TO_DATE(?,'dd/mm/yyyy')";
			try{
				depositInfo = runner.multipleSelection(sql,new String[]{opencoverNo,startDate,endDate});
			}
			catch(Exception e){e.printStackTrace();}
			return depositInfo;
	}
