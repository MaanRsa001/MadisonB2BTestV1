...
public class PolicyGenerationService 
{
	...
	public String quoteGeneration(String refNo, WebServicePolicyInfo info, String[][] commodityInfo,String belongingBranch)
	{
		...
		taxInfo=getTaxInfo(info.getBranchCode(), info.getLoginId(), info.getIssuer());
		 if(!info.isEndt() && !CollectionUtils.isEmpty(taxInfo)){	
			...
			info.setPolicyFee(calculatePolicyFee(info.getBranchCode(), info.getProductId(), info.getOpenCoverPolicyNo(), info.getTotalPremium(), info.getExcessPremium(), StringUtils.isBlank(policyFeeStatus)?"":policyFeeStatus, info.getBrokerType()));
	...
	public String generateQuoteSchedule(WebServicePolicyInfo info)
	{
		...
		try{
			...
			js.certificateSchedule(applicationNo, branchCode, belongingBranch, filePath, "PRINT","","","Y");			
		}catch (Exception e) {
			e.printStackTrace();
			LogManager.debug("Exception @ {"+e.getMessage()+"}");
		}
			...
			String brokerCode, String loginBranch, String promotinalCode, String belongingBranch,String packageType)
    {
		 HashMap<String, Object> commodityRates = new HashMap<String, Object>();
	   	 ...
	   	 String baseRate="", rate = "", warRate="", discountPercentage="", totalPremium="", totalMarine="", totalWar="", totalSaleterm="", totalTolerance="", policyExcess="",excessRate="";
	...
	public String calculateGovernmentTax(double totalMarineWar,String excessPremium,String loginId, String openCoverNo) {
		...
		try{
			...
			if(!policyDao.isOpenCoverDeposit(openCoverNo)) {
				...
				String govTaxContant=runner.singleSelection("SELECT  NVL(GOVT_TAX, 0) GOVT_TAX FROM BROKER_MASTER WHERE AGENCY_CODE=(SELECT OA_CODE FROM LOGIN_MASTER WHERE LOGIN_ID=?)",new String[]{loginId});
