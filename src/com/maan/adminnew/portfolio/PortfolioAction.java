...
public class PortfolioAction extends ActionSupport implements ModelDriven<PortfolioBean> {
	...
	public String getList()
	{
		...
		portfolioList=pservice.getPortFolio(bean.getFromdate(), bean.getTodate(), bean.getRep(), bean.getProductID(), (StringUtils.isBlank(bean.getBranchId())?branchCode:bean.getBranchId()), "", "");
		return "portfolio";
	}
	public String getListAjax() {
		if("portfolioLists".equals(bean.getReqFrom())){
		    	portfolioList=pservice.getPortFolio(bean.getFromdate(), bean.getTodate(), bean.getRep(), bean.getProductID(), (StringUtils.isBlank(bean.getBranchId())?branchCode:bean.getBranchId()), "", "");
			bean.setOptionMode("portfolioLists");
		}
