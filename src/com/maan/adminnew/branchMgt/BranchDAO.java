...
public class BranchDAO extends MyJdbcTemplate{
	Logger logger =LogUtil.getLogger(BranchDAO.class);
	...
	public List<Object> getCountriesNew() {
		List<Object> countryList=null;
		try{
			logger.info("Enter to The getCountriesNew()");
			String sql=getQuery("BR_GET_COUNTRIES_NEW");
			logger.info("getCountriesNew() Query=> "+sql);
			countryList = this.mytemplate.queryForList(sql);		    	
			logger.info("Exit The getCountriesNew()");
		}catch (Exception e) {
			e.printStackTrace();
			logger.info("Exception in  The getCountriesNew()");

		}
		return countryList;
	}
	public List<Object> getCurrenciesNew() {
		List<Object> currencyList=null;
		try{
			logger.info("Enter to The getCurrenciesNew()");
			String sql=getQuery("BR_GET_CURRENCIES_NEW");
			logger.info("getCurrenciesNew() Query=> "+sql);
			currencyList = this.mytemplate.queryForList(sql);		    	
			logger.info("Exit The getCurrenciesNew()"+sql);
		}catch (Exception e) {
			e.printStackTrace();
			logger.info("Exception in  The getCurrenciesNew()");

		}
		return currencyList;
	}
	public List<Object> getBelongingBranchNew() {
		List<Object> result=null;
		logger.info("Enter into the getBelongingBranchNew()");
		try{
			String sql=getQuery("BR_GET_BELONG_BRCH_NEW");
			logger.info("getBelongingBranchNew() Query=> "+sql);
			result=this.mytemplate.queryForList(sql);
		logger.info("Exit into the getBelongingBranchNew()");
		}catch (Exception e) {
			e.printStackTrace();
		logger.info("Enter into the getBelongingBranchNew()"+e);	
		}
		return result;
	}
	public void loadLostNew(BranchBean bean) {
		logger.info("Enter into the loadLostNew()");
		try{
			String sql=getQuery("BR_LOAD_LIST_NEW");
			logger.info("loadLostNew Query >>"+sql);
			bean.setBranchList(this.mytemplate.queryForList(sql));

		}catch (Exception e) {
			logger.info("Execption into the loadLostNew()"+e);	 
		}
		logger.info("Exit into the loadLostNew()");

	}
	public void getSelectedBranch(BranchBean bean) {
		logger.info("Enter into the getSelectedBranch()");
		try{
			String sql=getQuery("BR_GET_SEL_BROKER_NEW");
			Object []args={bean.getBranchCode()};
			logger.info("getSelectedBranch Query::: "+queryFrammer(sql, args));
			logger.info("Args:"+StringUtils.join(args,","));
			Map<String,String> record = this.mytemplate.queryForMap(sql, args);
			bean.setBranchCode(record.get("BRANCH_CODE")==null?"":record.get("BRANCH_CODE").toString());
			bean.setBranchName(record.get("BRANCH_NAME")==null?"":record.get("BRANCH_NAME").toString());
			...
			bean.setLang(record.get("LANG_YN")==null?"":record.get("LANG_YN").toString());
			bean.setBelongingBranch(record.get("BELONGING_BRANCH")==null?"":record.get("BELONGING_BRANCH").toString());
			bean.setPolicyFee(record.get("POLICY_FEE_PERCENT")==null?"":String.valueOf(record.get("POLICY_FEE_PERCENT")));
			 
			logger.info("Exit into the getSelectedBranch()");
		}catch (Exception e) {
			e.printStackTrace();
			logger.info("Exception into the getSelectedBranch()"+e);
		}
	}
	public boolean saveBranchNew(BranchBean bean) {
		boolean result=false;
		try{
			logger.info("Enter to The saveBranchNew()");
			String sql=getQuery("BR_SAVE_BRANCH_NEW");
			Object []args={bean.getBranchName(),
					bean.getStatus(),bean.getAddress1()
					,bean.getAddress2(),bean.getAddress3(),
					bean.getCity(),bean.getCountry(),
					...
					bean.getHeaderName(),bean.getFooterName(),
					bean.getSignImageName(),bean.getStampImageName(),
					bean.getCurrency(),bean.getCurrency(),bean.getOriginCountry(),
					bean.getDestiCountry(),"",
					bean.getDecimalPlaces(),bean.getTax(),
					bean.getBranchPrefix(),bean.getLang(),bean.getPolicyFee()				
			};
			logger.info("saveBranchNew Query:: "+queryFrammer(sql, args));
			this.mytemplate.update(sql,args);
			result=true;
			logger.info("Exit to The saveBranchNew()");
		}catch (Exception e) {
			e.printStackTrace();
			logger.info("Exception in The saveBranchNew()");
		}
		return result;
	}
	public boolean updateBranchNew(BranchBean bean) {
		boolean result=false;
		logger.info("Enter into updateBranchNew()");
		try{
			String sql=getQuery("BR_UPDATE_BRANCH_NEW");
			Object []args={bean.getBranchName(),
					bean.getStatus(),bean.getAddress1()
					,bean.getAddress2(),bean.getAddress3(),
					bean.getCity(),bean.getCountry(),
					...
					bean.getDestiCountry(),"",
					bean.getDecimalPlaces(),bean.getTax(),
					bean.getBranchPrefix(),bean.getLang(),bean.getPolicyFee(),bean.getBranchCode()					
			};
			logger.info("updateBranchNew Query:: "+queryFrammer(sql, args));
			this.mytemplate.update(sql,args);
			result=true;
		}catch (Exception e) {
			e.printStackTrace();
			logger.info("Exception into updateBranchNew()"+e);
		}
		return result;
	}
	public boolean updateBranchOnly(BranchBean bean) {
		boolean result=false;
		logger.info("Enter into updateBranchOnly()");
		try{
			String sql=getQuery("BR_UPDATE_BRANCH_ONLY");
			Object []args={bean.getBranchName(),
					bean.getStatus(),bean.getAddress1()
					,bean.getAddress2(),bean.getAddress3(),
					bean.getCity(),bean.getCountry(),
			...
			logger.info("updateBranchOnly Query:: "+queryFrammer(sql, args));
			this.mytemplate.update(sql,args);
			result=true;
		}catch (Exception e) {
			e.printStackTrace();
			logger.info("Exception into updateBranchOnly()"+e);
		}
		return result;
	}
