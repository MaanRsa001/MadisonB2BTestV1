...
public class PDFDisplay 
{
	...
	public Map getPDFStatus(final String policyNo, final String loginCode, final String productId) throws BaseException
	{
		...
		LogManager.debug(ENTER);
			...
			if("3".equals(productId) || "11".equals(productId)) {
				...
				if ("admin".equalsIgnoreCase(userType)){
					sql = "select pdf_admin_status from home_position_master where policy_no=? and status='P'";
				} else {
					sql = "select pdf_broker_status from home_position_master where policy_no=? and status='P'";
				}
			}
	...
	public void getUpdatePDFStatus(final String policyNo, final String loginCode,final String clickStatus, String productId) throws BaseException
	{
		...
		String sql;
			...
			if("3".equals(productId) || "11".equals(productId)) {
				...
				if ("admin".equalsIgnoreCase(userType))	{
					sql = "update home_position_master set pdf_admin_status=? where policy_no=? and status='P'";
				} else {
					sql = "update home_position_master set pdf_broker_status=? where policy_no=? and status='P'";
				}
			}
