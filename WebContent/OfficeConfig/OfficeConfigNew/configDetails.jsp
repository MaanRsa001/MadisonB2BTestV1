					...
					<div class="row">
						...
						<div class="panel panel-primary">
							<div class="panel-heading">
								<s:if test='configType == "config"'>
									<s:text name="Edit Configuration Details"/>
								</s:if>
								<s:elseif test='configType == "formula"'>
									<s:text name="Edit Formula Details"/>
								</s:elseif>
							</div>
							<div class="panel-body">
												...
												<tbody>
													<s:iterator begin="1" end="add" var="list" status="stat">
														<tr>
															...
															<td>
																<s:select name="displayOrderConfig[%{#stat.count-1}]" id="displayOrderConfig[%{#stat.count-1}]" cssClass="form-control inputBoxS" headerKey="" headerValue="None" list="displayOrderList" listKey="DISPLAY_ORDER" listValue="DISPLAY_ORDER" ></s:select>
															</td>
															<td>
								...
								<s:elseif test='configType == "formula"'>
										...
										<div class="col-xs-12" style="overflow:scroll;width:100%;overflow:auto">
											<table class="footable" width="100%">
												<thead>
													...
													<tr>
														<td>
															<s:textarea cols="100" rows="8" class="inputBoxA" name="formula" maxlength="1000" onkeyup="return ismaxlength(this)"/>
														</td>
														<td>
															<s:textarea cols="100" rows="8" class="inputBoxA" name="remarks" maxlength="1000" onkeyup="return ismaxlength(this)"/>
														</td>
														<td>
															<s:radio list="#{'Y':'Active','N':'Deactive'}" name="status" id="status" />
														</td>
													</tr>
												<tbody>
												</tbody>
									...
									<div class="col-xs-12 col-sm-12 col-md-12" align="center">
										<button type="button" class="btn btn-sm btn-danger" value="Back" onclick="funBack();">Back</button>
										<button type="button" class="btn btn-sm btn-success" value="Submit" onclick="funSubmitFormula('add');">Submit</button>
									</div>
								</s:elseif>
							</div>
						</div>
...
function addMore(mode) {
	...
	document.configDtl.action="editConfigOfsMaster.action";
	document.configDtl.submit();
}

function funSubmit(mode){
	document.configDtl.mode.value=mode;
	document.configDtl.action="updConfigOfsMaster.action";
	document.configDtl.submit();
}

function funBack(){
	document.configDtl.mode.value="list";
	//document.configDtl.action="coverageDtlListOfsMaster.action";
	document.configDtl.action="fieldDetailOfsMaster.action";
	document.configDtl.submit();
...
function funSubmitFormula(mode){
	document.configDtl.mode.value=mode;
	document.configDtl.action="updFormulaOfsMaster.action";
	document.configDtl.submit();
}
</script>
</body>
