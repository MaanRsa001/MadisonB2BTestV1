...
<html>
	...
	<body>
	<s:form id="fields" name="fields" action="" theme="simple">
							...
							<div class="panel-body">
											...
											<s:if test='mode == "add"'>
												<div class="col-xs-12 col-sm-6 col-md-4 col-lg-4">
													<div class="form-group">
				    									<label><s:text name="Field Name" /><font color="red">*</font></label>
				    									<s:select name="fieldName" id="fieldName" cssClass="form-control inputBoxS" list="coverageExcludedList" listKey="COVERAGES_ID" listValue="COVERAGES_NAME" headerKey="" headerValue="---select---"></s:select>
													</div>
												</div>
											</s:if>
											<s:else>
								...
								<div class="panel panel-primary">
									<div class="panel-heading">
										<div class="row">
											<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
												<label><s:text name="Default Broker Configuration"/></label>
											</div>
										</div>
									</div>
									<div class="panel-body">
								...
								<div class="row">
									<div class="col-xs-12 col-sm-12 col-md-12" align="center">
										...
										<s:submit cssClass="btn btn-sm btn-success" value="Submit" onclick="funSubmitAdd();"></s:submit> &nbsp;&nbsp;&nbsp;
									</div>
								</div>
							</div>
						</div>
										...
										<div class="row">
											<table class="display responsive no-wrap" id="gridTable" width="100%" cellspacing="0">
												...
												<tr>
													...
													<td><s:text name="Config"/></td>
													<td><s:text name="Formula"/></td>
												</tr>
												</thead>
												<tbody>
													<s:iterator value="coverageIncludedList" var="list" status="stat">
														<tr>
															...
															<td style="text-align: center;">
																<button type="button" class="btn btn-warning btn-sm" onclick="funConfig('edit','<s:property value="#list.COVERAGES_ID"/>','<s:property value="#list.COVERAGES_NAME"/>');"><i class="fa fa-pencil-square-o"></i></button>
															</td>
															<td style="text-align: center;">
																<button type="button" class="btn btn-warning btn-sm" onclick="funFormula('edit','<s:property value="#list.COVERAGES_ID"/>','<s:property value="#list.COVERAGES_NAME"/>');"><i class="fa fa-pencil-square-o"></i></button>
															</td>
														</tr>
													</s:iterator>
												</tbody>
...
function funEdit(mode,id,name){
	document.fields.mode.value=mode;
	document.fields.coverageId.value=id;
	document.fields.coverageLabelName.value=name;
	var e = document.getElementById("contentId");
	var desc = e.options[e.selectedIndex].text;
	document.fields.contentLabelName.value=desc;
	document.fields.action="editFieldDtlOfsMaster.action";
	document.fields.submit();
}
function funBackProd(){
	document.fields.mode.value="list";
	document.fields.action="schemeListOfsMaster.action";
	document.fields.submit();
}
function funBack(){
	document.fields.mode.value="list";
	document.fields.action="fieldDetailOfsMaster.action";
	document.fields.submit();
}
function funSubmitAdd(){
	document.fields.action="updateFieldOfsMaster.action";
	document.fields.submit();
}
...
function funConfig(mode,id,name){
	...
	var desc = e.options[e.selectedIndex].text;
	document.fields.contentLabelName.value=desc;
	document.fields.action="editConfigOfsMaster.action";
	document.fields.submit();
}
function funFormula(mode,id,name){
	...
	var desc = e.options[e.selectedIndex].text;
	document.fields.contentLabelName.value=desc;
	document.fields.action="editFormulaOfsMaster.action";
	document.fields.submit();
}
</script>
</body>
