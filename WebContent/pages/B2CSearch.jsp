...
<body> 
<s:form name="search" theme="simple" action="b2cSearchReport.action">
			...
			<div class="container">
				<div class="panel panel-primary">
					...
					<div class="panel-body">
						...
						<div style="color: red;"><b id="errordtl"></b></div>
						<br class="clear" />
						<div class="row">
							<div class="col-xs-12 col-sm-12 col-md-6 col-lg-4">
								<div class="text">
									<b>Quote No<font color="red">*</font></b>
								</div>
								<div class="tbox">
									<%-- <s:textfield name="searchValue" id="searchValue" onkeyup="getSearchResult('searchResult',this.value)" cssClass="form-control"/> --%>	
									<s:textfield name="quoteNo" id="quoteNo" cssClass="form-control numericOnly"/>
								</div>
							</div>
							<div class="col-xs-12 col-sm-12 col-md-6 col-lg-4">
								<div class="text">
									<b>Mobile No<font color="red">*</font></b>
								</div>
								<div class="tbox">
									<%-- <s:textfield name="searchValue" id="searchValue" onkeyup="getSearchResult('searchResult',this.value)" cssClass="form-control"/> --%>	
									<s:textfield name="mobileNo" id="mobileNo" cssClass="form-control numericOnly" maxlength="10"/>
								</div>
							</div>
							<br class="clear" />
						</div>
...
function getSearchResult(id, searchValue) {
	...
	var quotenum=document.search.quoteNo.value;
	var mobilenum=document.search.mobileNo.value;
	if('User'=='<s:property value="#session.usertype"/>'){
		loginId='<s:property value="#session.user"/>'; 
	} else {
		loginId=document.search.loginId.value;
	}
	//alert(quotenum+" , "+mobilenum);
	if(quotenum=='') {
		error+='<s:text name="Please Enter Quote No" /><br>';
	} 
	if(mobilenum=='') {
		error+='<s:text name="Please Enter Mobile No" /><br>';
	} 
	if(loginId=='') {
		error+='<s:text name="error.loginId.empty" /><br>';
	}
	//alert(error);
	document.getElementById("errordtl").innerHTML=error;
	if(error=='' ) {
		postRequest('${pageContext.request.contextPath}/b2cSearchReport.action?quoteNo='+quotenum+'&loginId='+loginId+'&mobileNo='+mobilenum+'&searchFrom=S',id);
	} /*else if(error!='') {
		alert(error);
	}*/
}
