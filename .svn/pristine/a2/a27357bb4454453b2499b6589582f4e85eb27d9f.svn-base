...
/**
 * MultipartRequestParser reads a multipart/form-data HTTP request, writes any
 * files to disk, and stores all web variables in a hashmap.
 * 
...
public class MultipartRequestParser {
	 ...
	 *            ServletRequest
	 * @param saveDirectory
	 *            Directory to which to save files
	 * @exception IOException
	 */
	public MultipartRequestParser(ServletRequest request, String saveDirectory)
			throws IOException {
	...
	/**
	 * Gets the boundary marker, moves past the first boundary, and calls
	 * processParts() to process each part of the request.
	 * 
	 * @exception Exception
	 */
	...
	/**
	 * Reads the name and value of the web variable and places it in the web
	 * variable hashmap.
	 * 
	 * @param name
	 *            String containing the name of the variable to store
	 * @exception IOException
	 */
	...
	/**
	 * Reads a file from the multipart/form-data request and writes it to the
	 * file system.
	 * 
	 * @param disposition
	 *            ContentDisposition object containing the name of the web
	...
	private void storeFile(ContentDisposition disposition) throws IOException {
		...
		try {
			...
			File file = new File(saveDirectory + disposition.getFilename());
	...
	/**
	 * Reads the name of web variable and filename from the Content-Disposition
	 * header and stores it in a new ContentDisposition object.
	 * 
	 * @return ContentDisposition object with name of web variable and filename
	 * @exception Exception
	 */
	...
	/**
	 * Returns the web variable name from the Content-Disposition header.
	 * 
	 * @param line
	...
	private String getDispositionName(String line) {
		...
		String dispositionName = line.substring(nameIndex, line.indexOf("\"",
				nameIndex + 1));
	...
	private String getDispositionFilename(String line) {
		...
		String filename = line.substring(filenameIndex, line.indexOf("\"",
				filenameIndex));
	...
	/**
	 * MultipartStreamReader is an inner class that contains a single readLine()
	 * method. This method returns a single line from the request input stream.
	 */
	class MultipartStreamReader {
		/**
		 * Reads a single line (up to 1K in length) from the input stream. The
		 * line is limited to 1K in length since this method is for reading HTTP
		 * headers and web variable values, not files.
		 * 
	...
	/**
	 * ContentDisposition is an inner class that encapsulates the properties of
	 * the Content-Disposition HTTP header. Specifically, this class stores the
	 * name of the web variable and the filename.
	 */
	class ContentDisposition {
		 ...
		 * 
		 * @param name
		 *            String containing the name of the web variable
		 */
