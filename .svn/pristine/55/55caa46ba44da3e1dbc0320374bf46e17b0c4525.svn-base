...
<head>
	...
	<%
		...
		String subCoverageType = request.getParameter("subCoverage");
		subCoverageType = subCoverageType == null ? "" : subCoverageType;
		...
		String subCoverageValue = coverageBean.getIDRelatedValue(
				"COVERAGES_NAME", "OFS_MASTER", "COVERAGES_ID",
				subCoverageType);
	
		String[][] values = new String[0][0];
		values = subCoverageBean.selectValue(productID, coverageID,
				schemeID, contentTypeID, subCoverageType);
	
		String displayOrderList = "";
		displayOrderList = subCoverageBean.getSubCoverageDisplayOrderList(
				productID, contentTypeID, schemeID, coverageID);
	
		if (values.length > 0 && error.length()<=0) {
			displayOrder = values[0][3]==null?"":values[0][3];
			controlType = values[0][2]==null?"":values[0][2];
			coverageLimit = values[0][0]==null?"":values[0][0];
			subRate = values[0][1]==null?"":values[0][1];
			calculationType = values[0][5]==null?"":values[0][5];
			status = values[0][4]==null?"":values[0][4];
			effectiveDate = values[0][6]==null?"":values[0][6];
			rsaCode = values[0][7]==null?"":values[0][7];
			subCoverageLimit = values[0][8]==null?"":values[0][8];
		}
	%>
</head>
<body>
<div id="page" class="content">	
	<div class="body">
	<form name="form1" action="SubCoverageController" method="post">
	<div class="row">
		<div class="col-xs-12">
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					<div class="row">
						...
						<div class="col-xs-12">
							<div class="textV">Sub Coverage Name</div>
							<div class="tboxV">&nbsp;:&nbsp;<%=subCoverageValue%></div>
						</div>
					</div>
					<br class="clear" />
					<div class="row">
									...
									<div class="row">
										<div class="col-xs-12">
											<table class="footable" width="100%">
												...
												<tr>
													...
													<td width="60%">
														<select name="displayOrder" class="inputBoxS">
															...
															<%	int plusone = 0;
																	for (int i = 1; i < 100; ++i) {
																		if ((displayOrderList.indexOf(i+",") != -1) || displayOrder.equalsIgnoreCase(i + "")) {
															%>
																			<option value="<%=i%>"
															...
															<option value="<%=plusone+1%>"> <%=plusone+1%> </option>							
														</select>
													</td>
												</tr>
												<tr style="">
													<td class="content_align">
														Control Type
													</td>
													<td class="content_align">
														<%
														String[] CONTROL_TYPE = { "Display", "textbox", "Radio", "dropdown","textarea" };
														%>
														<select name="controlType"  class="inputBoxS">
															<%
															for (int i = 0; i < CONTROL_TYPE.length; ++i) {
															%>
															<option value="<%=CONTROL_TYPE[i]%>"
																<%=CONTROL_TYPE[i].equalsIgnoreCase(controlType) ? "selected"
																: ""%>>
																<%=CONTROL_TYPE[i]%>
															</option>
															<%
												...
												<tr>
													<td>Sub Coverage Limit</td>
													<td><input type="text" name="coverageLimit" class="inputBox" value="<%=coverageLimit%>" maxlength="50"></td>
												</tr>
												<tr>
													<td>Sub Coverage Limit(Value)</td>
													<td><input type="text" name="subCoverageLimit" class="inputBox" value="<%=subCoverageLimit%>" maxlength="10"></td>
												</tr>
												<tr>
													<td>Sub Rate</td>
													<td><input class="inputBox" type="text" name="subRate" value="<%=subRate%>" maxlength="10" ></td>
												</tr>
												<tr>
												...
												<tr>
													...
													<td>
														<input type="text" name="effectiveDate" value="<%=effectiveDate%>" size="30" maxlength="20" class="inputBox datepicker" size="10" id="effectiveDate" />
													</td>
												</tr>
												<tr>
													<td>Core Code</td>
	...
	<input type="hidden" name="subCoverage" value="<%=subCoverageType%>">
	<input type="hidden" name="requestForm" value="subCoverageInsert">
	</form>
	</div>
</div>
</body>
