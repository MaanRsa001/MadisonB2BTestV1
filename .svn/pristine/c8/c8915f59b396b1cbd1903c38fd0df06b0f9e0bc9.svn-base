...
public class finalprint
{
	...
	{
		LogManager.push("getPolictDetails=====>");
		LogManager.debug(ENTER);
		String obj[]={};
			String sql;
			if (queryStatus.equalsIgnoreCase(QUOTENO))
...
//					sql22 = "select clauses_description from clauses_master where(cover_id in('"+coverId+"') and  extra_cover_id=?) and clauses_id not in("+conExtraIds+") and status in ('Y','R') and branch_code=? order by clauses_id";
	...
	{
		LogManager.info("getPdf - Enter || applicationNo: "+applicationNo);
		String result[][];
		...
		String sql="";
		if("CLAUSES".equalsIgnoreCase(clauses)){
			  sql="select DISTINCT(CODEDESC),CODESTATUS,CODE from table(schedule_clauses(?,?,?,''))";
		}else{
			 sql = "Select CODEDESC,CODESTATUS,CODE from (SELECT CODEDESC,CODESTATUS,CODE FROM TABLE(VIEWCLAUSES('Clauses',?,?)) union all SELECT CODEDESC,CODESTATUS,CODE FROM TABLE(VIEWCLAUSES('War',?,?))) WHERE CODESTATUS IS NOT NULL";
		}
		final String args[] = {applicationNo,branchCode,applicationNo,branchCode};
		result = runner.multipleSelection(sql,args);
		LogManager.info("getPdf - Exit || Result: "+result);
		return result;
	}
	
	public String[][] getOpenCoverPdf(String proposalNo, String branchCode, String belongingBranch) throws BaseException 
	{
		LogManager.info("getOpenCoverPdf - Enter || proposalNo: "+proposalNo);
		String result[][];
	...
	{
		LogManager.info("Get SurveyClause - Enter || PolicyNo: "+getPolNo+" LoginId:"+login);
		...
		LogManager.info("Get SurveyClause - Exit || Result: "+result);
		return 	result;
	}
	...
	public String getHomeQuoteNo(String policyNo) {
		String result=runner.singleSelection("SELECT QUOTE_NO FROM HOME_POSITION_MASTER WHERE POLICY_NO=? AND STATUS in ('P','D')",new String[]{policyNo});
		return 	result;
	}
}// Class
