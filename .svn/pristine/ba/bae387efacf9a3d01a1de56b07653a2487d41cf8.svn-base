...
public class BranchService {
	...
	public void setDropDownNew(BranchBean bean) {
		bean.setCountryList(dao.getCountriesNew());
		bean.setCurrrencyList(dao.getCurrenciesNew());
		bean.setBelongingBranchList(dao.getBelongingBranchNew());
		bean.setCityList(getCities(bean.getCountry()==null?"":bean.getCountry()));
	}
	public void loadListNew(BranchBean bean) {
		dao.loadLostNew(bean);
	}
	public void getSelectedBranch(BranchBean bean) {
		dao.getSelectedBranch(bean);
	}
	public boolean saveBranchNew(BranchBean bean,String filePath) {
		boolean result=false;
		List<String> errorList=new ArrayList<String>();
		
			if("add".equalsIgnoreCase(bean.getMode())){
				uploadImages(bean,filePath);
				if(dao.saveBranchNew(bean))
					
					result=true;
			}else{
				if("Y".equalsIgnoreCase(bean.getImgMode())){
					uploadImages(bean,filePath);
					if(dao.updateBranchNew(bean))
						result=true;
				}else{
					if(dao.updateBranchOnly(bean))
						result=true;
				}
			}		  
		return result;	
