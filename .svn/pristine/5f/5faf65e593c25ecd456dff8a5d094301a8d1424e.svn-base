...
public class RatingEngineAction extends ActionSupport implements ModelDriven<RatingEngineBean> {
	...
	/*public List<String> getCommcategList(){
		return rservice.getCommcategList(branchCode);
	}*/

	/*public List<Object> getBaseRateList() {		
	...
	public String getTList() {
		LogManager.push("ENTER-->Method to getTList");
		//transportList = rservice.getAdminTransportList(branchCode);
		LogManager.info("getTList() - Exit" + login_id);
		return "transportList";
	}
		...
		if(!hasActionErrors()){
			...
			if("edit".equals(bean.getMode())){
				...
				if(bo==false){
					...
					Object[] val={bean.getCountryID(),country_short_name,bean.getEff_date(),bean.getStatus(),bean.getWarRate(),endPlace,ep,country_nat,remarks,sp,startPlace,bean.getCode(),bean.getCountryDetID(),branchCode};
					string="update";
					...
					LogManager.info("val.args===>" + StringUtils.join(val, ", "));
					string="insert";
					rservice.getUpdateCountryDet(val,string);
				}
	...
	public String updateConstantMaster() {
		...
		try {
			...
			LogManager.debug("EXCEPTION @ updateConstantMaster { " + exception + " } ");
		}
		return forward;
	}
	...
	public void validatebaserateCalculate1() throws NumberFormatException {
		try {		
			...
			LogManager.info("Conversion Error"+e);
		}		
	}		
	public void validatebaserateCalculate(String commodity,String destCountryId,double localSumInsured,String branch) throws NumberFormatException {
		...
		LogManager.info("commodityReferal===>" +commodityReferal );		
		LogManager.info("sumInsuredReferal===>" +sumInsuredReferal );	
		LogManager.info("countryReferal===>" +countryReferal );	
	...
	public String updateExclusion() {
		...
		if (!hasActionErrors()) {
			if("add".equals(bean.getMode()))
			{
				...
				LogManager.info("val.args===>" + StringUtils.join(val, ", "));				rservice.getNewExclusion(val);
	...
	public void validateNewCountry() throws NumberFormatException {
		...
		catch(NumberFormatException e)
		{
			...
			LogManager.info("Conversion Error");
		}
	}
	...
	public void validatenewError() throws NumberFormatException {
		try {
			...
			if (StringUtils.isBlank(bean.getErrorDesc()))
				...
				for (int i = 0; i < j; i++) {
					Object k = aobj[i];
					LogManager.info((new StringBuilder("val.args11===>"))
							.append(k).toString());
				}
	...
	public String updateError() {
		...
		if (!hasActionErrors()) {
			...
			{		
				...
				for (int i = 0; i < j; i++) {
					Object k = aobj[i];
					LogManager.info((new StringBuilder("val.args===>")).append(k)
							.toString());
				}
			...
			try {
				...
				if("edit".equals(bean.getMode()))
				{
					Object val[] = { bean.getClauseDesc(),
							...
							bean.getDisplayorder(),bean.getRemarks(), pdfName, bean.getRsacode(),
	...
	public String editWsrcc(){
		...
		if(!hasActionErrors()){
			...
			try {
				...
				LogManager.debug("Exception @ editWsrcc { " + ex + " } ");
			}
	...
	public String editclause(){
		try {
			...
			LogManager.debug("Exception @ editclause() { " + ex + " } ");
		}
		return "editClauseID";
	}
	...
	public boolean validateDate(String val){		
		...
		try{
			...
			LogManager.info(date2.compareTo(date1)+"");
			if(date2.compareTo(date1) < 0)
				bo=true;
		...
		if(StringUtils.isNotBlank(bean.getReqFrom())){
			try  {
				...
				if(getActionErrors().size()<=0) {
					...
					LogManager.info("UPLOAD EXCEL FILE PATH===>"+(ServletActionContext.getServletContext().getRealPath(getText("EXCEL_UPLOAD_PATH")))+"/"+uploadFileName);
					String csvfilename="";
					...
					LogManager.info("UPLOAD CSV PATH NAME===>"+(ServletActionContext.getServletContext().getRealPath(getText("CSV_UPLOAD_PATH")))+"/"+csvfilename);
	...
	public String download(){
		LogManager.info("download() - Enter");
		try {
			if("exchangeupload".equals(bean.getReqFrom())){
	...
	public String downloadTrans(){
		LogManager.info("downloadTrans() - Enter || menuType: "+menuType+ " tranId: "+tranId);
		try {
			String typeId=getText(bean.getReqFrom());
			Map<Object,Object> downloadInfo=service.getDownloadInfo(typeId, tranId, downloadType);
			List<Object> recordsList = service.getDownloadData(tranId, downloadInfo, downloadType);
		...
		LogManager.info("downloadTrans() - Exit");
		return "download";
	}
	...
	public String updateCommodity() {
		...
		if (!hasActionErrors()) {
			...
			if("add".equals(bean.getMode())){
				Object[] val = {bean.getCommodityName(), bean.getCommodityRates(), bean.getCoverage_Referal(),
			...
			else
			{
				...
				if(bo==false)  {							
					...
					Object[] val = {bean.getCommodityName(),  bean.getCommodityRates(), bean.getCoverage_Referal(),ICC_A_Sea_Rate, ICC_B_Sea_Rate, ICC_C_Sea_Rate,
					...
					string="insert";
					Object[] val = {       bean.getCommodityID(), bean.getCommodityName(), bean.getCommodityRates(), bean.getCoverage_Referal(),
							ICC_A_Sea_Rate, ICC_B_Sea_Rate, ICC_C_Sea_Rate, ICC_C_ND_Sea_Rate,
							ICC_A_Frozen_Meat_Sea_Rate,ICC_A_Frozen_Food_Sea_Rate,ICC_C_Frozen_Meat_Sea_Rate,ICC_C_Frozen_Food_Sea_Rate                       
							,Icc_Air_Cargo_Air_Risks,Icc_Air_Cargo_All_Risks,All_Risks_Land_Transit,
							Land_Transit_land,All_Risks_parcel_post,All_Risks_Sea_and_Air, "", "", "", "", remarks,
