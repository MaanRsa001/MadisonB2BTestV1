...
<html>
	...
	<s:set var="attachmentDocList" value="%{attachmentDocList}"/>
		...
		function docupload() {
				...
				if(error.length > 0) {
					...
					document.form1.action = "${pageContext.request.contextPath}/submitAttachmentDoUpload.action";
		...
		function downloadDoc(filePath,sFileName) {
			...
			document.form1.action = "${pageContext.request.contextPath}/downloadAttachmentDoUpload";
			document.form1.submit();
		}
	function deleteDoc(filePath,docId) {
		var decision = confirm("File will be deleted. Do You Want to continue? ","");
			if (decision==true){
				document.getElementById('filePath').value = filePath;
				document.form1.action = "${pageContext.request.contextPath}/deleteAttachmentDoUpload.action?docId="+docId;
				document.form1.submit();
				}else{
					document.form1.action = "${pageContext.request.contextPath}/emailAttachmentDoUpload.action";
					document.form1.submit();
				}
			return false;
		}
